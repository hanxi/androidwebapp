...
function cookieHelper() {
    ...
    this.getCookie = function(cname) {
        ...
        if (window.android) {
            cookie = window.android.getCookie().toLocaleString();
        }
...
function cheesyWordGetter() {
    this.words = ['不错哦', '真厉害', '保持这水平继续', '你很叼哦', '屌爆了', '继续加油'];
...
function Game() {
    ...
    this.resetGame = function() {
        ...
        $('#levelDescriptor').html("等级 " + this.level);
        setTimeout(function(){
            $('#newLevel').modal('show');
            self.setupLevel();
        }, 500);
    ...
    this.updateCounts = function() {
        $(".currLevel").html("当前等级: <b>" + this.level + "</b>");
        $(".score").html("当前点击次数: <b>" + this.currentClicks +"</b>");
        $(".best").html("最高等级: <b>" + this.bestLevel + "</b> (" + this.clicksForBest + " 次点击)");
        $(".total").html("总点击次数: <b>" + this.totalClicks + "</b>");
    }
